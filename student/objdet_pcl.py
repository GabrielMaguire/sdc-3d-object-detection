...
def show_range_image(frame, lidar_name):
    ...
    img_intensityensity = ri_intensity.astype(np.uint8)

    ri_elongation = ri_elongation * 255 / (np.amax(ri_elongation) - np.amin(ri_elongation))
    img_elongation = ri_elongation.astype(np.uint8)

    ri_nlz = ri_nlz * 255
    img_nlz = ri_nlz.astype(np.uint8)

    # step 6 : stack the range and intensity image vertically using np.vstack and convert the result to an unsigned 8-bit integer
    img_range_intensity = np.vstack((img_range, img_intensityensity, img_elongation, img_nlz))
